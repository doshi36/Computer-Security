...
def encrypt(message_file, key_file, output_file):
    ...
    while (bv.more_to_read):
        ...
        if bitvec._getsize() > 0:
            ...
            final_string += (RE + LE)
            new_str = final_string.get_hex_string_from_bitvector()
...
def decrypt(encrypted_file, key_file, output_file):
    key = get_encryption_key(key_file)
    round_keys = (generate_round_keys(key))
    round_keys.reverse()

    filr = open(encrypted_file, "r")
    for t in filr:
        encrypted = BitVector(hexstring = t)
    
    for i in range (0, len(encrypted) // 64 ):
        bitvec = encrypted[64*i:64*(i+1)]
        ...
        if bitvec._getsize() > 0:
            ...
            #final_string = BitVector(size = 0)
            final_string = (RE + LE)
            new_str = final_string.get_text_from_bitvector()
            output_file.write(new_str)

if __name__ == "__main__":
    if sys.argv[1] == '-e':
        encrypted_txt = open(sys.argv[4],'w')
        encrypt(sys.argv[2], sys.argv[3], encrypted_txt)

    if sys.argv[1] == '-d':
        decrypted_txt = open(sys.argv[4],'w')
        decrypt(sys.argv[2], sys.argv[3], decrypted_txt)