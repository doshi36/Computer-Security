...
# to out_file. No return value is required.
    ...
    line1 = image_fp.readline()
    line2 = image_fp.readline()
    line3 = image_fp.readline()
    list_iv = []
    ...
    for i in range (len(list_iv)):
        plaintext_bv = bv[128*i:128*(i+1)]
        image_bitvec = encrypt(BitVector(intVal = list_iv[i], size = 128), key_file)
        ciphertext_block = image_bitvec ^ plaintext_bv
